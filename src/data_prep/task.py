...
class data_prep():
    ...
    def prepare_data(self) -> pd.DataFrame:
        ...
        if self.target_variable in self.raw_data.columns:
            processed_data = dp_t.prepare_target_variable(data = processed_data , target_variable = self.target_variable , config = self.config)
...
if __name__ == "__main__":
    ...
    with open('config.yaml') as f:
        ...
        try:
            ...
            sys.stdout.write("analysis_read_log: Data properly read" + "|Time of run:" +
                  str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))

        except Exception as e:
            sys.stderr.write("analysis_read_log: data read failed |error message:"+ str(e) + "|Time of run:" +
                     str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))
        ...
        try:
            ...
            data_quality_details['target_variable_statistics'] = processed_data[config['target_variable']].describe()
            data_quality_details['categoric_price_count'] = processed_data['price_category'].count()
            sys.stdout.write("data_preparation_log: Data prepared and cleaned" + "|Time of run:" +
                  str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))
        
        except Exception as e:
            sys.stderr.write("data_preparation_log: data preparation failed |error message:"+ str(e) + "|Time of run:" +
                     str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))

        try:
            
            load = load(processed_data = processed_data, config = config, data_details = data_quality_details)
            load.store_locally_data()
            sys.stdout.write("data_storage_log: Data successfully stored" + "|Time of run:" +
                  str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))

        except Exception as e:
            sys.stderr.write("data_storage_log: local data storage failed |error message:"+ str(e) + "|Time of run:" +
                     str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())))
