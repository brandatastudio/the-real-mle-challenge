...
def data_quality_details(raw_data:pd.DataFrame) -> dict:
    ...
    quality_details = {"data_shape" : data_shape , "data_columns" : data_columns ,
                     "data_statistics" : data_statistics , "data_missing_values" : data_missing_values}

    return(quality_details)


def remove_threshold_of_numeric_column(data:pd.DataFrame ,numeric_column:str = 'price' , threshold:int = 10) -> pd.DataFrame:
...
def prepare_target_variable(data:pd.DataFrame, target_variable:str , config:dict) -> pd.DataFrame:
    ...
    if processed_data[target_variable].dtype is not numeric_dtypes:
            processed_data[target_variable] = processed_data[target_variable].str.extract(r"(\d+).").astype(int)
    

    
    processed_data = remove_threshold_of_numeric_column(data = processed_data,
                                                        numeric_column = target_variable, threshold =10)

    processed_data['price_category'] = pd.cut(processed_data[target_variable], bins=[10, 90, 180, 400, np.inf], labels = [int(float(x))for x in [*config['tar_variable_category_mapping']]])
...
def binary_columns_from_text(data:pd.DataFrame ,column_to_check:str,
                     columns_names:list ,  columns_text_to_check:list) -> pd.DataFrame:
    ...
    if len(columns_text_to_check) != len(columns_names):
        raise ValueError('columns_text_to_check and columns_names should be same size lists')
        return(data)

    else:
        
        for i in range(0,len(columns_names)):
            data[columns_names[i]] = data[column_to_check].str.contains(columns_text_to_check[i]).astype(int)
...
def prepare_features(data:pd.DataFrame, config:dict) -> pd.DataFrame:
    ...
    processed_data =  binary_columns_from_text(data = processed_data ,column_to_check = config['text_column_for_binary_generation'],
