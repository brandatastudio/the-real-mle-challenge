...
class read:
    ...
    def get_model_details(self) -> dict:
        winning_model_details = pickle.load(
            open(config["training"]["ml_model_details_path"], "rb")
        )
    ...
    def get_data(self, sep: str = ","):
        ...
        processed_data = pd.read_csv(
            filepath_or_buffer=self.config["training"]["prepped_for_Ml_data_path"],
            sep=sep,
        )
        raw_data = pd.read_csv(
            filepath_or_buffer=self.config["data_prep"]["raw_data_path"], sep=sep
        )
...
if __name__ == "__main__":
    with open("config.yaml") as f:
        try:
            ...
            sys.stdout.write(
                "explainability read log: Data and winning model properly read"
                + "|Time of run:"
            ...
            sys.stderr.write(
                "explainability read log: data and winning model read failed |error message:"
                + str(e)
                + "|Time of run:"
                + str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()))
            )
        ...
        try:
            ...
            exp_t.plot_histogram(
                data=processed_data,
                winning_model_details=winning_model_details,
                column_to_plot="price",
            )
            exp_t.plot_neighbourhood(data=processed_data)

            sys.stdout.write(
                "explainability plotting log: plots made correctly"
                + "|Time of run:"
                + str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()))
            )
            ...
            sys.stderr.write(
                "explainability plottinh log: plotting failed |error message:"
                + str(e)
                + "|Time of run:"
                + str(strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime()))
            )
