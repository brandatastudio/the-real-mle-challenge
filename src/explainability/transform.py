...
def plot_confusion_matrix(winning_model_details:dict , config:dict):
    ...
    classes = [int(float(x))for x in [*config['data_prep']['tar_variable_category_mapping']]]
    
    labels = [x for x in config['data_prep']['tar_variable_category_mapping'].values()]

    conf = confusion_matrix(winning_model_details['y_test'], winning_model_details['y_pred'])
    conf = conf / conf.sum(axis=1).reshape(len(classes), 1)
    fig, ax = plt.subplots(figsize=(10, 10))
    sns.heatmap(conf, annot=True, cmap='BuGn', square=True, fmt='.2f', annot_kws={'size': 10}, cbar=False)
...
def plot_histogram(data:pd.DataFrame , winning_model_details:dict, column_to_plot:str):
    ...
    if column_to_plot != 'y_pred':
        ...
        ax.hist(winning_model_details[column_to_plot], bins=range(0, max(winning_model_details[column_to_plot]), 10))
    ax.grid(alpha=0.2)
...
def plot_neighbourhood(data:pd.DataFrame):
    ...
    MAP_NEIGHB = {1: "Bronx",2 : 'Queens', 3 : "Staten Island" ,  4: "Brooklyn" , 5: "Manhattan"}
